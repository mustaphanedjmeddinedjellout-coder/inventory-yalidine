...
All endpoints below are mounted under `/api/store` and require the header `X-STORE-KEY`.
...
Environment variables used by Store API:
- `STORE_API_KEY` — secret key checked against `X-STORE-KEY`
- `CORS_ALLOWED_ORIGINS` — comma-separated origins allowed by CORS (include Noire domain + localhost)
